...
const SYSTEM_INSTRUCTION = (targetRole, targetCompany, difficulty) => `You are an experienced technical interviewer conducting a mock interview for a ${targetRole} position${targetCompany ? ` at ${targetCompany}` : ''}.
...
const getNextQuestion = async ({ targetRole, targetCompany, difficulty, messages }) => {
  if (MOCK_MODE) {
    return `Mock question #${messages.length + 1} for ${targetRole}`;
  }
  const instruction = SYSTEM_INSTRUCTION(targetRole, targetCompany, difficulty);
