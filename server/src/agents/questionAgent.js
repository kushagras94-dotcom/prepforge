...
const SYSTEM_INSTRUCTION = (targetRole) => `You are an experienced technical interviewer conducting a mock interview for a ${targetRole} position.
...
const getNextQuestion = async ({ targetRole, messages }) => {
  ...
  const instruction = SYSTEM_INSTRUCTION(targetRole);
